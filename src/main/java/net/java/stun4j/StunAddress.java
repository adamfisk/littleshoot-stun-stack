...
public final class StunAddress
    {
     ...
     */
    public StunAddress(final String hostname, final int port)
        {
        m_socketAddress = new InetSocketAddress(hostname, port);
        }
     ...
     * address.
     */
    public StunAddress(final byte[] ipAddress, final int port)
        {
        ...
        try
            ...
            address = new InetSocketAddress(InetAddress
                .getByAddress(ipAddress), port);
            }
        catch (final UnknownHostException ex)
            {
            //Unknown Host - Let's skip resolution
            address = new InetSocketAddress((ipAddress[0] & 0xFF) + "."
                + (ipAddress[1] & 0xFF) + "." + (ipAddress[2] & 0xFF) + "."
                + (ipAddress[3] & 0xFF) + ".", port);
            }
     ...
     */
    public StunAddress(final InetAddress address, final int port)
        {
        m_socketAddress = new InetSocketAddress(address, port);
        }
    ...
    public StunAddress(int port)
        {
        m_socketAddress = new InetSocketAddress(port);
        }
    ...
    public byte[] getAddressBytes()
        {
        return (m_socketAddress == null ? null : m_socketAddress.getAddress()
            .getAddress());
        }
    ...
    public char getPort()
        {
        return (m_socketAddress == null ? 0 : (char) m_socketAddress.getPort());
        }
    ...
    public String toString()
        {
        return "STUN ADDRESS: "+ m_socketAddress.toString();
        }
    ...
    public boolean equals(Object obj)
        ...
        final StunAddress target = (StunAddress) obj;
        if (target.m_socketAddress == null && m_socketAddress == null)
            return true;

        return m_socketAddress.equals(target.getSocketAddress());
        }
    
    public int hashCode()
        {
        return 17 * m_socketAddress.hashCode();
        }
    ...
    public String getHostName()
        {
        return m_socketAddress.getHostName();
        }
    }